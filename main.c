...
void loadfile() {
	...
	if (dsrlnk(&pabdat, VDP_PAB)) {
        // program image load failed, try a record based load (DF128)
        pabdat.OpCode = DSR_OPEN;
        pabdat.Status = DSR_TYPE_FIXED|DSR_TYPE_DISPLAY|DSR_TYPE_INPUT|DSR_TYPE_SEQUENTIAL;
        pabdat.VDPBuffer = VDP_BUFFER;	// address of the data buffer in VDP memory
        pabdat.RecordLength = 128;		// size of records
        pabdat.CharCount = 0;			// number of bytes read or number of bytes to write (per record)
        ...
        if (dsrlnk(&pabdat, VDP_PAB)) {
            ...
            // it opened, so read in the records
            unsigned int adr = VDP_BUFFER;
            unsigned char rec = 0;
            
            while (adr < VDP_BUFFER+8192) {
                pabdat.OpCode = DSR_READ;
                pabdat.Status = DSR_TYPE_FIXED|DSR_TYPE_DISPLAY|DSR_TYPE_INPUT|DSR_TYPE_SEQUENTIAL;
                pabdat.VDPBuffer = adr;	        // address of the data buffer in VDP memory
                pabdat.RecordLength = 128;		// size of records
                pabdat.CharCount = 0;			// number of bytes read or number of bytes to write (per record)
                pabdat.RecordNumber = rec;	    // record number for normal files, available bytes (LOAD or SAVE) for PROGRAM type
                pabdat.ScreenOffset = 0;		// Used in BASIC for screen BIAS. Also returns file status on Status call. (DSR_STATUS_xxx)
                pabdat.NameLength = 0;			// for this implementation only, set to zero to read the length from the string
                pabdat.pName = name;			// for this implementation only, must be a valid C String even if length is set
                
                if (dsrlnk(&pabdat, VDP_PAB)) break;
...
int main() {
	...
	writestring(16,5,"Build date: 4/21/2024");
